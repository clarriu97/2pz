...
from pipeline.models import Branch, CatchmentDemand, Zone, ZoneActivity
from pipeline.sourcing.demand import COMPONENT_WEIGHTS
from pipeline.util import clamp01, haversine_m, log_minmax


def _closed_ring(h3_index: str) -> list[list[float]]:
    """The cell boundary as a valid GeoJSON ring.

    `h3.cell_to_boundary` returns six vertices; a GeoJSON linear ring must
    repeat the first vertex to close. Some renderers tolerate an open ring and
    others silently drop the polygon, so we close it here rather than rely on
    the consumer being forgiving.
    """
    ring = [[round(ln, 6), round(lt, 6)] for lt, ln in h3.cell_to_boundary(h3_index)]
    return [*ring, ring[0]]
...
def build_zones(branches: list[Branch], activity: dict[str, ZoneActivity]) -> list[Zone]:
    ...
    for idx in cells:
        ...
        res = counts.residential if counts else 0
        act = counts.activity if counts else 0
        aff = counts.affluence if counts else 0
    ...
    for idx, metro in cells.items():
        ...
        elif counts.residential == 0 and counts.activity > 0:
        ...
        zones.append(
            Zone(
                ...
                lat=round(lat, 6),
                lon=round(lon, 6),
                boundary=_closed_ring(idx),
                area_km2=round(cell_area, 3),
                residential_count=counts.residential if counts else 0,
                activity_count=counts.activity if counts else 0,
                affluence_count=counts.affluence if counts else 0,
...
def catchment_demand(
    branches: list[Branch], activity: dict[str, ZoneActivity]
) -> dict[str, CatchmentDemand]:
    ...
    """
    per_branch: dict[str, CatchmentDemand] = {}
    for b in branches:
        cells = []
        for idx, counts in activity.items():
            ...
            if haversine_m(b.lat, b.lon, lat, lon) <= b.catchment_radius_m:
                cells.append(counts)
        res = sum(c.residential for c in cells)
        act = sum(c.activity for c in cells)
        aff = sum(c.affluence for c in cells)
        n = max(len(cells), 1)
        composite = (
        ...
        per_branch[b.branch_id] = CatchmentDemand(
